...
class MediaLibraryManager: NSObject {
    ...
    // MARK: Properties
    /// The instance of `AuthorizationManager` used for looking up the current device's
    ///  Media Library and Cloud Services authorization status.
    let authorizationManager: AuthorizationManager
    /// The instance of `MPMediaPlaylist` that corresponds to the playlist created by this
    /// sample in the current device's Media Library.
    ...
    init(authorizationManager: AuthorizationManager) {
        ...
        // from the `AuthorizationManager`, `MPMediaLibrary` and `UIApplication`.
        let notificationCenter = NotificationCenter.default
        notificationCenter.addObserver(self,
                                       selector:
                                        #selector(handleAuthorizationManagerAuthorizationDidUpdateNotification),
    ...
    func createPlaylistIfNeeded() {
        ...
        if let playlistUUIDString = userDefaults.string(forKey: MediaLibraryManager.playlistUUIDKey) {
            // In this case, the sample already created a playlist in a previous run.
            // In this case we lookup the UUID that was used before.
            ...
            playlistCreationMetadata.descriptionText =
                                "This playlist was created using " +
                                "\(Bundle.main.infoDictionary!["CFBundleName"]!) to demonstrate how " +
                                "to use the Apple Music APIs"
        ...
        MPMediaLibrary.default().getPlaylist(with: playlistUUID,
                                             creationMetadata: playlistCreationMetadata) {(playlist, error) in
