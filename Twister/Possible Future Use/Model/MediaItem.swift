...
class MediaItem {
    ...
    /// The various keys needed for serializing an instance of `MediaItem` using a JSON
    /// response from the Apple Music Web Service.
    struct JSONKeys {
    ...
    init(json: [String: Any]) throws {
        ...
        guard let artworkJSON = attributes[JSONKeys.artwork] as? [String: Any],
              let artwork = try? Artwork(json: artworkJSON) else {
