...
class Artwork {
    // MARK: Types
    /// The various keys needed for serializing an instance of `Artwork` using a
    /// JSON response from the Apple Music Web Service.
    struct JSONKeys {
    ...
    let width: Int
    /**
     The string representation of the URL to request the image asset. This template should be used to create
     the URL for the correctly sized image your application wishes to use.
     See `Artwork.imageURL(size:)` for additional information.
     */
    ...
    func imageURL(size: CGSize) -> URL {
        /*
         There are three pieces of information needed to create the URL for the image we want for a given size.
         This information is the width, height and image format.  We can use this information in addition to the
         `urlTemplateString` to create the URL for the image we wish to use.
         */
