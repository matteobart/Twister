...
class SettingsTableViewController: UITableViewController, MFMailComposeViewControllerDelegate {
    ...
    func updateLabels() {
        ...
        authSpotifyLabel.text = spotifyManager.hasToken ? "Deauthorize Spotify" : "Authorize Spotify"
        spotifyImageView.image = spotifyManager.hasToken ?
                        UIImage(systemName: "checkmark.square.fill") : UIImage(systemName: "questionmark.square.fill")
        spotifyImageView.tintColor = spotifyManager.hasToken ? appTint : .systemYellow
    }
    ...
    func toggleAppleMusic() {
        if authorizationManager.isAuthenticated() { // apple music
            let alert = UIAlertController(title: "Apple Music is Authorized",
            ...
            let alert = UIAlertController(title: "Apple Music has been denied",
                                          message: """
                                                   ...
                                                   Twister's settings -> find 'Allow Twister to Access' -> make \
                                                   sure 'Media & Apple Music' is turned on
                                                   """,
                                          preferredStyle: .alert)
            ...
            let alert = UIAlertController(title: "Twister will Request Access",
                                          message: """
                                                   For Twister to work, we must request access to your music \
                                                   library. The following prompt will ask you to give access to \
                                                   Twister.
                                                   """,
                                          preferredStyle: .alert)
    ...
    func toggleSpotify() {
        if spotifyManager.hasToken {
            let alert = UIAlertController(title: "Deauthorize Spotify",
                                          message: "Are you sure you would like to deauthorize Spotify?",
                                          preferredStyle: .alert)
    ...
    func openFAQ() {
        var range = NSRange()
        let an3 = NSMutableAttributedString(string: """
                                                    Possibly! If you have any recommendations feel free to reach \
                                                    out to us by email at matteodevapps@gmail.com
                                                    """)
        ...
        let an4 = NSMutableAttributedString(string: """
                                                    Oh no! Be sure to bring him outside and don't squash him. \
                                                    Let us know by emailing matteodevapps@gmail.com
                                                    """)
        ...
        let an5 = NSMutableAttributedString(string: """
                                                    If you're a developer or graphic designer or think you can \
                                                    be valuable feel free to check out the code for this app \
                                                    available at www.github.com/matteobart/Twister
                                                    """)
        ...
        let items = [FAQItem(question: "Do you need an account for Apple Music and Spotify?",
                             answer: """
                                    Yes! You will need an account for both services. However you do not need \
                                    a premium Spotify account to transfer playlists to or from Spotify.
                                    """),
                     FAQItem(question: "Why does the process take so long?",
                            answer: """
                                    iTunes currently limits you to search for 20 songs per minute. This means \
                                    that if your playlist contains 300 songs this will take 15 minutes to convert \
                                    to Apple Music. Playlists to Spotify should be near instantaneous.
                                    """),
